...
*** Exception: AAAA!!!!
> 12345^54321
огромное число, не помещающееся целиком в консоли
> 2 < 3 || 9999954321^99912345 > 12345^54321
True
-}
...
5
it :: Integer
(0.00 secs, 858720 bytes)
> 5 < 7
True
it :: Bool
(0.00 secs, 848872 bytes)
> if 2 > 3 then 7 else 5
5
it :: Integer
(0.01 secs, 867716 bytes)
> 5 > 6 && False
False
it :: Bool
(0.00 secs, 0 bytes)
...
-- (объясните смысл типовой аннотации: Запись "1 :: Num a => a" означает, что выражение "1" имеет тип "a", где "a" принадлежит
-- классу типов Num (имеет экземпляр класса типов Num, является числовым типом).)
double :: Num a => a -> a
double a = a * 2
